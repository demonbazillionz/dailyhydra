...
class HydrationViewModel(private val repository: HydrationRepository) : ViewModel() {
    ...
    fun loadDailyHydrationTip(force: Boolean = false) {
        viewModelScope.launch {
            ...
            val goal = state.dailyGoalMls
            val todayIntake = state.totalIntakeToday
            val progress = state.progressPercent
            val streakVal = state.streak.currentStreak

            val prompt = """
                You are a friendly, highly professional science-backed hydration expert. Provide a short, actionable daily hydration tip personalized for a user.
                Here is the user's current context:
                - Daily Water Goal: ${goal}ml
                - Current Today's Intake: ${todayIntake}ml (progress: ${progress}%)
                - Current Hydration Streak: ${streakVal} days
                - Date: ${todayDateStr}

                Requirements:
                1. Keep the tip very short, engaging, and science-backed (1 to 2 sentences maximum).
                2. Highlight a specific health benefit, biological mechanism, or fascinating hydration fact.
                3. Address them professionally and encouragement-wise.
                4. Do NOT use markdown bold/italic formatting. Return plain, polished text only.
                5. Do NOT use generic advice like 'drink 8 glasses of water'. Be specific or provide a fascinating hydration/biological fact.
            """.trimIndent()

            try {
                val apiKey = com.example.BuildConfig.GEMINI_API_KEY
                if (apiKey.isBlank() || apiKey == "MY_GEMINI_API_KEY") {
                    // Fallback to a nice local tip if API key isn't provided/valid yet
                    val localTips = listOf(
                        "Water supports cell regeneration and keeps your skin looking radiant and healthy.",
                        "Drinking water before meals can help boost your metabolism by up to 30% for over an hour.",
                        "Fatigue is often one of the first signs of mild dehydration. Keep sipping to stay sharp!",
                        "Water is essential for joint lubrication; keeping hydrated helps protect your cartilage and spinal cord.",
                        "Your brain is 73% water. Staying hydrated improves concentration, reaction times, and mental clarity."
                    )
                    val fallbackTip = localTips.random()
                    _hydrationTipText.value = fallbackTip
                    repository.saveSetting("hydration_tip_text", fallbackTip)
                ...
                val request = com.example.api.GenerateContentRequest(
                    contents = listOf(
                        com.example.api.Content(
                            parts = listOf(
                                com.example.api.Part(text = prompt)
                            )
                        )
                    ),
                    ...
                    systemInstruction = com.example.api.Content(
                        parts = listOf(
                            com.example.api.Part(text = "You are a professional, science-backed daily hydration assistant. You do not return any conversational filler, markdown formatting, or introductory phrases. Only return the requested 1-2 sentence tip.")
                        )
                    )
                )

                val response = com.example.api.GeminiRetrofitClient.service.generateContent(apiKey, request)
                val tip = response.candidates?.firstOrNull()?.content?.parts?.firstOrNull()?.text?.trim()
                ...
                _hydrationTipError.value = "Failed to fetch. Tap to retry."
